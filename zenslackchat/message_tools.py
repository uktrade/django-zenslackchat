...
def message_who_is_on_call(on_call, slack_client, chat_id, channel_id):
    ...
    """
    if on_call != {}:
        message = f"📧 Primary on call: {on_call['primary']}\n" f"ℹ️ Secondary on call: {on_call['secondary']}."
        post_message(slack_client, chat_id, channel_id, message)


def message_issue_zendesk_url(slack_client, zendesk_uri, ticket_id, chat_id, channel_id):
...
_resolve_cmds = ["resolve", "resolve ticket", "close", "🆗", "✅"]
