...
def test_slack_oauth_successful_token_exchange(
    requests_get, settings, log, db
):
...
def test_zendesk_oauth_successful_token_exchange(
    requests_post, log, db
):
...
@patch('zenslackchat.views.requests.post')
def test_pagerduty_oauth_successful_token_exchange(
    requests_post, settings, log, db
):
    ...
    factory = APIRequestFactory()
    request = factory.get(
        '/pagerduty/oauth/', dict(code=9876, subdomain='frog')
    )
    with patch.dict('webapp.settings.__dict__', settings, clear=True):
        response = views.pagerduty_oauth(request)
