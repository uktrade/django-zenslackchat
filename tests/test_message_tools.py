...
@patch("zenslackchat.message_tools.post_message")
def test_message_whois_on_call_not_posted(post_message):
    ...
    app_token = {"token_type": "fake", "access_token": "totally fake token"}
    message_who_is_on_call(
...
@patch("zenslackchat.message_tools.post_message")
@patch("zenslackchat.models.PagerDutyApp.get")
def test_message_who_is_on_call(mock_get, post_message, db, log):
    """Test the message sent / not sent for who is on call."""
    channel_id = "slack-channel-id"
    chat_id = "slack-chat-id"
    slack_client = MagicMock()
    ...
    mock_get.return_value = loads()

    message = "📧 Primary on call: Fred Sprocket\n" "ℹ️ Secondary on call: Tony Tiger."
    ...
    post_message.assert_called_with(
        slack_client, "slack-chat-id", "slack-channel-id", message
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("2020-09-08T16:35:14Z", _utc(datetime(2020, 9, 8, 16, 35, 14))),
    ],
)
...
def test_truncate_email(email_body, truncated, character_limit):
    assert message_tools.truncate_email(email_body, character_limit) == truncated
